...
export function HeaderMenu({ layoutProps }) {
  ...
  return (
    <div
      className={`header-menu header-menu-mobile ${layoutProps.ktMenuClasses} justify-content-between`}
    >
      <div>
        {/* <SearchDropdown /> */}
        <h6 className="mb-0 font-weight-bold">
          ¡Una nueva forma de{" "}
          <strong style={{ color: "#fc8330" }}>vivir tu seguridad!</strong>
        </h6>
      </div>
      <div className="d-flex" style={{ gap: "10px", width: "304px" }}>
        <Link
          to="/homepage"
          className="btn btn-secondary w-100 font-weight-bold"
          style={{
            background: 'url("/img/btn-ventajas.png") no-repeat 100%/100%',
          }}
        />
        ...
        <button
          ...
          style={{
            background: 'url("/img/btn-descarga.png") no-repeat 100%/100%',
            height: "40px",
          }}
        />

        {modal && <QRModal setModal={setModal} />}
      </div>
    </div>
  );
